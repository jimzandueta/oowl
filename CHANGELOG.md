...
- **Dispatcher permission leak into artifact-owner agents.** Changed the dispatcher's `docs/specs/**` permission from `deny` to `ask`. OpenCode inherits parent `deny` rules into Task child sessions (last-match-wins), so the dispatcher's `docs/specs/**` deny was overriding the artifact owners' own `docs/specs/**/<artifact>.md` allows — blocking `architect`, `planner`, `designer`, and `reviewer` from writing `design.md`, `implementation.md`, `ui-spec.md`, and `review.md`.
